...
{
  init();
  //post((char *)(Stk::rawwavePath() + "sinewave.raw").c_str());
}
...
/*
  ...
  if ( lastSamples < samples ) {
    ...
    //maxmsp
    //data_ = t_getbytes(samples * sizeof(StkFloat));
  }
  if ( lastChannels < channels_ ) {
    if ( lastOutputs_ ) delete [] lastOutputs_;
    lastOutputs_ = (StkFloat *) new StkFloat[channels_];
    //lastOutputs_ = t_getbytes(samples * sizeof(StkFloat));
  }
...
bool WvIn :: getMatInfo( const char *fileName )
{
  ...
  /* maxmsp; can't figure out how to get this one to compile
  if ( strstr(head, "0") {
