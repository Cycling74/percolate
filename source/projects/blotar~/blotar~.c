...
void blotar_perform64(
	t_blotar* x, t_object* dsp64, double** ins, long numins, double** outs, long numouts, long sampleframes, long flags, void* userparam);


/****FUNCTIONS****/

void pluck(t_blotar* x, float amplitude, float position) { /* this function gets interesting here, */
...
void blotar_perform64(
	...
	while (n--) {
		...
		temp     = filterRatio * temp + filterRatioInv * OneZero_tick(&x->lowpass, (x->boreDelay.lastOutput + tempsave));
		...
		if (!x->mode)
			pressureDiff = JetTabl_lookup(pressureDiff + (er * temp));    // from the original, "incorrect" blotar
		else {
			pressureDiff = JetTabl_lookup(pressureDiff) + (er * temp);    // this is "correct"
			// constrain? to keep it from blowing up
			if (pressureDiff > 10.)
				pressureDiff = 0.;    // make this threshold user-controllable
