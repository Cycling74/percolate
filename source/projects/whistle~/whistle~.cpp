...
t_class* whistle_class;

typedef struct _whistle {
	// header
	t_pxobject x_obj;
...
// args that the user can input, in which case whistle_new will have to change
void* whistle_new(long num_inputs, long num_outputs);
void  whistle_free(t_whistle* x);
void  whistle_assist(t_whistle* x, void* b, long m, long a, char* s);

// dsp stuff
void whistle_dsp64(t_whistle* x, t_object* dsp64, short* count, double samplerate, long maxvectorsize, long flags);
void whistle_perform64(
	t_whistle* x, t_object* dsp64, double** ins, long numins, double** outs, long numouts, long sampleframes, long flags, void* userparam);

// for getting floats at inputs
void whistle_float(t_whistle* x, double f);

// for custom messages
void whistle_setpower(t_whistle* x, t_symbol* s, long argc, t_atom* argv);
void whistle_controlchange(t_whistle* x, t_symbol* s, long argc, t_atom* argv);
void whistle_noteon(t_whistle* x, t_symbol* s, long argc, t_atom* argv);
void whistle_noteoff(t_whistle* x, t_symbol* s, long argc, t_atom* argv);
void whistle_settype(t_whistle* x, t_symbol* s, long argc, t_atom* argv);
...
void ext_main(void* p) {
	t_class* c = class_new("whistle~", (method)whistle_new, (method)whistle_free, (long)sizeof(t_whistle), 0L, A_DEFLONG, A_DEFLONG, 0);

	class_addmethod(c, (method)whistle_assist, "assist", A_CANT, 0);
	class_addmethod(c, (method)whistle_dsp64, "dsp64", A_CANT, 0);
	class_addmethod(c, (method)whistle_float, "float", A_FLOAT, 0);

	// our own messages
	class_addmethod(c, (method)whistle_setpower, "power", A_GIMME, 0);
	class_addmethod(c, (method)whistle_controlchange, "control", A_GIMME, 0);
	class_addmethod(c, (method)whistle_noteon, "noteon", A_GIMME, 0);
	class_addmethod(c, (method)whistle_noteoff, "noteoff", A_GIMME, 0);
	class_dspinit(c);
...
void* whistle_new(long xD, long yD) {
	int i;

	// leave this; creates our object
	t_whistle* x = (t_whistle*)object_alloc(whistle_class);

	// zero out the struct, to be careful (takk to jkclayton)
	if (x) {
		for (i = sizeof(t_pxobject); i < sizeof(t_whistle); i++)
			((char*)x)[i] = 0;
	}
...
void whistle_controlchange(t_whistle* x, t_symbol* s, long argc, t_atom* argv) {
	...
	if (argc < 2) {
		post("whistle~ error: need two arguments, control number and control value\n");
		return;
	}
	...
	x->myWhistle->controlChange(temp[0], temp[1]);
}
...
void whistle_noteon(t_whistle* x, t_symbol* s, long argc, t_atom* argv) {
	...
	if (argc < 2) {
		post("whistle~ error: need two arguments, frequency and amplitude\n");
		return;
	}
	...
	x->myWhistle->noteOn(temp[0], temp[1]);
}
...
void whistle_dsp64(t_whistle* x, t_object* dsp64, short* count, double samplerate, long maxvectorsize, long flags) {
	...
	object_method(dsp64, gensym("dsp_add64"), x, whistle_perform64, 0, NULL);
}
...
void whistle_perform64(
	...
	if (x->x_obj.z_disabled || (x->power == 0))
		return;
