...
void whistle_dsp(t_whistle *x, t_signal **sp, short *count); 
t_int *whistle_perform(t_int *w);
...
//for custom messages
void whistle_setpower(t_whistle *x, Symbol *s, short argc, Atom *argv);
void whistle_controlchange(t_whistle *x, Symbol *s, short argc, Atom *argv);
void whistle_noteon(t_whistle *x, Symbol *s, short argc, Atom *argv);
void whistle_noteoff(t_whistle *x, Symbol *s, short argc, Atom *argv);
void whistle_settype(t_whistle *x, Symbol *s, short argc, Atom *argv);
...
extern "C" int main(void)
{
    ...
    setup((struct messlist **)&whistle_class, (method)whistle_new, (method)whistle_free, (short)sizeof(t_whistle), 0L, A_DEFLONG, A_DEFLONG, 0);
   
	//standard messages; don't change these  
    addmess((method)whistle_dsp, "dsp", A_CANT, 0);
    addmess((method)whistle_assist,"assist", A_CANT,0);
    
    //our own messages
    addmess((method)whistle_setpower, "power", A_GIMME, 0);
    addmess((method)whistle_controlchange, "control", A_GIMME, 0);
    addmess((method)whistle_noteon, "noteon", A_GIMME, 0);
    addmess((method)whistle_noteoff, "noteoff", A_GIMME, 0);
    
    //so we know what to do with floats that we receive at the inputs
    addfloat((method)whistle_float);
...
void *whistle_new(long xD, long yD)
{
    ...
    t_whistle *x = (t_whistle *)newobject(whistle_class);
...
void whistle_dsp(t_whistle *x, t_signal **sp, short *count)
{
	...
	dsp_addv(whistle_perform, (x->num_inputs + x->num_outputs + 2), dsp_add_args); //add them to the signal chain
