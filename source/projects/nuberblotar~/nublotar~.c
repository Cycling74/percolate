...
void nublotar_perform64(
	t_nublotar* x, t_object* dsp64, double** ins, long numins, double** outs, long numouts, long sampleframes, long flags, void* userparam);
...
void nublotar_assist(t_nublotar* x, void* b, long m, long a, char* s) {
	...
	if (m == 1) {
		...
		if (a == 10)
			sprintf(s, "input %ld: input signal", a);
	}
	if (m == 2) {
		for (i = 0; i < x->num_outputs; i++)
			if (a == i)
...
void nbody_int(t_nublotar* x, int f) {
	...
	if (f < 1) {
		...
		if (x->instrument == GUITAR1)
			nbody_guitar1(x, 0.);
		if (x->instrument == GUITAR2)
			nbody_guitar2(x, 0.);
		if (x->instrument == VIOLIN)
			nbody_violin(x, 0.);
		if (x->instrument == HARDANGER)
			nbody_hardanger(x, 0.);
		if (x->instrument == MANDOLIN)
			nbody_mandolin(x, 0.);
		if (x->instrument == ARCHTOP)
			nbody_archtop(x, 0.);
	}
}
...
void nublotar_perform64(t_nublotar* x, t_object* dsp64, double** ins, long numins, double** outs, long numouts, long sampleframes,
