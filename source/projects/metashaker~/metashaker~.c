...
void metashake_dsp64(t_metashake* x, t_object* dsp64, short* count,
    double samplerate, long maxvectorsize, long flags);
void metashake_perform64(t_metashake* x, t_object* dsp64, double** ins,
...
float tamb_tick(t_metashake* x)
{
    ...
    if (my_random(1024) < x->num_objects) {
        ...
        x->coeffs1[0] = -TAMB_CYMB_RESON * 2.0 * cos((x->res_freq1 + x->cymb_rand) * TWO_PI / x->srate);
        x->cymb_rand = noise_tick() * x->res_freq2 * x->res_random; //* 0.05;
        x->coeffs2[0] = -TAMB_CYMB_RESON * 2.0 * cos((x->res_freq2 + x->cymb_rand) * TWO_PI / x->srate);
    }
    x->input = x->sndLevel;
...
float sleigh_tick(t_metashake* x)
{
    ...
    if (my_random(1024) < x->num_objects) {
        ...
        x->coeffs[0] = -SLEI_CYMB_RESON * 2.0 * cos((x->res_freq + x->cymb_rand) * TWO_PI * x->one_over_srate);
        x->cymb_rand = noise_tick() * x->res_freq1 * 0.03;
        x->coeffs1[0] = -SLEI_CYMB_RESON * 2.0 * cos((x->res_freq1 + x->cymb_rand) * TWO_PI * x->one_over_srate);
        x->cymb_rand = noise_tick() * x->res_freq2 * 0.03;
        x->coeffs2[0] = -SLEI_CYMB_RESON * 2.0 * cos((x->res_freq2 + x->cymb_rand) * TWO_PI * x->one_over_srate);
        x->cymb_rand = noise_tick() * x->res_freq3 * 0.03;
        x->coeffs3[0] = -SLEI_CYMB_RESON * 2.0 * cos((x->res_freq3 + x->cymb_rand) * TWO_PI * x->one_over_srate);
        x->cymb_rand = noise_tick() * x->res_freq4 * 0.03;
        x->coeffs4[0] = -SLEI_CYMB_RESON * 2.0 * cos((x->res_freq4 + x->cymb_rand) * TWO_PI * x->one_over_srate);
    }
    x->input = x->sndLevel;
    x->input *= noise_tick(); // Actual Sound is Random
    x->input1 = x->input;
...
{
    t_class* c = class_new("metashaker~", (method)metashake_new, (method)dsp_free,
        (long)sizeof(t_metashake), 0L, A_DEFFLOAT, 0);
...
void metashake_assist(t_metashake* x, void* b, long m, long a, char* s)
{
    if (m == ASSIST_INLET) {
        switch (a) {
        case 0:
            sprintf(s, "(signal/float) number of objects");
            break;
        case 1:
        ...
        case 6:
            ...
            break;
        case 7:
            sprintf(s, "(signal/float) resonance frequency 5");
            break;
        case 8:
...
void metashake_int(t_metashake* x, int f) { metashake_float(x, (double)f); }
