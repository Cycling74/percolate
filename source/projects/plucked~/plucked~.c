...
void plucked_perform64(
	t_plucked* x, t_object* dsp64, double** ins, long numins, double** outs, long numouts, long sampleframes, long flags, void* userparam);
...
void pluck(t_plucked* x, float amplitude) {
	...
	for (i = 0; i < x->length; i++)
		DLineA_tick(&x->delayLine, x->delayLine.lastOutput + OnePole_tick(&x->pickFilt, Noise_tick()));
...
void plucked_perform64(
	...
	while (n--) {
		...
		temp   = DLineA_tick(&x->delayLine, OneZero_tick(&x->loopFilt, x->delayLine.lastOutput * x->loopGain));
		*out++ = temp * 3.;
	}
}
