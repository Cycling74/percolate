...
void tamb_perform64(
	t_tamb* x, t_object* dsp64, double** ins, long numins, double** outs, long numouts, long sampleframes, long flags, void* userparam);
...
float tamb_tick(t_tamb* x) {
	...
	if (my_random(1024) < x->num_objects) {
		...
		x->coeffs1[0] = -TAMB_CYMB_RESON * 2.0 * cos((x->res_freq + x->cymb_rand) * TWO_PI / x->srate);
