...
void vcomb_perform64(
	t_vcomb* x, t_object* dsp64, double** ins, long numins, double** outs, long numouts, long sampleframes, long flags, void* userparam);
...
void dline_setDelay(t_vcomb* x, long whichLine, long lag) {
	...
	while (x->outPoint[whichLine] < 0)
		x->outPoint[whichLine] += x->delaytime[whichLine];    // modulo maximum length
}
...
void vcomb_assist(t_vcomb* x, void* b, long m, long a, char* s) {
	...
	if (m == 1) {
		...
		if (a == 1)
			sprintf(s, "fft in 2");
		if (a == 2)
			sprintf(s, "delay time (int: in unit samples)");
		if (a == 3)
			sprintf(s, "feedback coefficient (float)");
	}
	if (m == 2) {
		for (i = 0; i < x->num_outputs; i++)
			if (a == 0)
...
void vcomb_perform64(
	...
	for (j = 0; j < n; j++) {
		...
		for (i = 0; i < x->num_inputs; i++) {
			if (x->in_connected[i])
				temp[i] = *in[i]++;    // use the signal vector if there is one
			else
				temp[i] = *in[i];    // otherwise use the global variable
		}
