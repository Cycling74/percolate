...
void vectorstretch_perform64(t_vectorstretch* x, t_object* dsp64, double** ins, long numins, double** outs, long numouts, long sampleframes,
...
void vectorstretch_perform64(t_vectorstretch* x, t_object* dsp64, double** ins, long numins, double** outs, long numouts, long sampleframes,
	...
	if (x->interp_method == INT_LINEAR) {
		for (i = 0; i < n; i++) {
			...
			if (stretchStart >= n)
				temp = 0.;    // requests for frames above max are set to 0.
			else
				temp = in[stretchStart];
