...
void PRC_Rev_dsp(t_PRC_Rev *x, t_signal **sp, short *count); 
t_int *PRC_Rev_perform(t_int *w);
...
//for custom messages
void PRC_Rev_setT60(t_PRC_Rev *x, Symbol *s, short argc, Atom *argv);
void PRC_Rev_setEffectMix(t_PRC_Rev *x, Symbol *s, short argc, Atom *argv);
void PRC_Rev_clearmesh(t_PRC_Rev *x, Symbol *s, short argc, Atom *argv);

void PRC_Rev_setpower(t_PRC_Rev *x, Symbol *s, short argc, Atom *argv);
...
void ext_main(void* p)
{
    ...
    setup((struct messlist **)&PRC_Rev_class, (method)PRC_Rev_new, (method)PRC_Rev_free, (short)sizeof(t_PRC_Rev), 0L, A_DEFLONG, A_DEFLONG, 0);
   
	//standard messages; don't change these  
    addmess((method)PRC_Rev_dsp, "dsp", A_CANT, 0);
    addmess((method)PRC_Rev_assist,"assist", A_CANT,0);
    
    //our own messages
    addmess((method)PRC_Rev_setpower, "power", A_GIMME, 0);
    addmess((method)PRC_Rev_setT60, "T60", A_GIMME, 0);
    addmess((method)PRC_Rev_setEffectMix, "mix", A_GIMME, 0);
    addmess((method)PRC_Rev_clearmesh, "clear", A_GIMME, 0);

    //so we know what to do with floats that we receive at the inputs
    addfloat((method)PRC_Rev_float);
...
void *PRC_Rev_new(long baseDelay, long yD)
{
    ...
    t_PRC_Rev *x = (t_PRC_Rev *)newobject(PRC_Rev_class);
...
void PRC_Rev_dsp(t_PRC_Rev *x, t_signal **sp, short *count)
{
	...
	dsp_addv(PRC_Rev_perform, (x->num_inputs + x->num_outputs + 2), dsp_add_args); //add them to the signal chain
