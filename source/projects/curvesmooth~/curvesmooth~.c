...
void curvesmooth_perform64(t_curvesmooth* x, t_object* dsp64, double** ins, long numins, double** outs, long numouts, long sampleframes,
...
void curvesmooth_assist(t_curvesmooth* x, void* b, long m, long a, char* s) {
	...
	if (m == 1) {
		for (i = 0; i < x->num_inputs; i++) {
			if (a == 0)
				sprintf(s, "target (signal)");
			if (a == 1)
				sprintf(s, "upsmooth time (signal/float)");
			if (a == 2)
				sprintf(s, "downsmooth time (signal/float)");
			if (a == 3)
				sprintf(s, "upsmooth coeff (signal/float)");
			if (a == 4)
				sprintf(s, "downsmooth coeff (signal/float)");
		}
	}
	if (m == 2) {
		for (i = 0; i < x->num_outputs; i++)
			if (a == 0)
				sprintf(s, "output (signal)");
	}
}
...
void curvesmooth_perform64(t_curvesmooth* x, t_object* dsp64, double** ins, long numins, double** outs, long numouts, long sampleframes,
	...
	while (n--) {    // this is where the action happens..... let's make something up
		...
		// assign the inputs
		if (x->in_connected[0])
			t = *in[0]++;    // use the signal vector if there is one
		else
			t = *in[0];    // otherwise use the global variable
		if (x->in_connected[1])
			T_up = (double)*in[1]++;
		else
			T_up = (double)*in[1];
		...
		if (!x->update_D) {    // when D is constant 1
			...
			// assign new current
			if (s > d)
				s = d;    // make sure we don't overshoot
			else if (s < -d)
				s = -d;
			x->c = c + s;
