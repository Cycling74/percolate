...
void munger_perform64(
	t_munger* x, t_object* dsp64, double** ins, long numins, double** outs, long numouts, long sampleframes, long flags, void* userparam);
...
float win_tick(t_munger* x, int whichone) {
	...
	while (x->winTime[whichone] >= (float)WINLENGTH)
		x->winTime[whichone] -= (float)WINLENGTH;
	while (x->winTime[whichone] < 0.)
		x->winTime[whichone] += (float)WINLENGTH;
...
float envelope(t_munger* x, int whichone, float sample) {
	...
	*/
	if (done < x->gvoiceRamp[whichone])
		sample *= (done * x->gvoiceOneOverRamp[whichone]);
	else if (tail < x->gvoiceRamp[whichone])
		sample *= (tail * x->gvoiceOneOverRamp[whichone]);
...
float newSetup(t_munger* x, int whichVoice, double frames) {
	...
	if (x->num_channels == 2) {
		...
		for (i = 0; i < x->num_channels; i++) {
			if (i == tmpDiscretePan)
				x->gvoiceSpat[whichVoice][i] = x->channelGain[i] + ((float)rand() - 16384.) * ONE_OVER_HALFRAND * x->channelGainSpread[i];
			else
				x->gvoiceSpat[whichVoice][i] = 0.;
		}
	}
	else {
		for (i = 0; i < x->num_channels; i++) {
			x->gvoiceSpat[whichVoice][i]
				= x->channelGain[i] + ((float)rand() - RAND_MAX * 0.5) * ONE_OVER_HALFRAND * x->channelGainSpread[i];
		}
	}
	...
	if (x->gvoiceSize[whichVoice] < 2. * x->rampLength) {
		...
		if (x->gvoiceRamp[whichVoice] <= 0.)
			x->gvoiceRamp[whichVoice] = 1.;
		...
		// 2. fixed positioning and moving buffer
			...
			if (x->gvoiceDirection[whichVoice] == 1) {    // going forward
				if (x->gvoiceSpeed[whichVoice] > 1.)
					...
					newPosition
						= x->recordCurrent - x->gvoiceSize[whichVoice] * x->gvoiceSpeed[whichVoice] - x->position * x->onethirdBufsize;
...
float newSize(t_munger* x, int whichOne) {
	...
	// set grain pitch
	if (x->smoothPitch == 1)
		x->gvoiceSpeed[whichOne] = x->gpitch + ((float)rand() - RAND_MAX * 0.5) * ONE_OVER_HALFRAND * x->gpitch_var;
	else {
	...
	if (x->gvoiceSpeed[whichOne] < MINSPEED)
		x->gvoiceSpeed[whichOne] = MINSPEED;
...
float newNote(t_munger* x, int whichVoice, int newNote, double frames) {
	...
	ADSR_setAllTimes(&x->gvoiceADSR[whichVoice], x->noteAttack[newNote], x->noteDecay[newNote], x->noteSustain[newNote],
		x->noteRelease[newNote], x->srate_ms);
...
void recordSamp(t_munger* x, float sample) {
	if (x->recordCurrent >= x->buflen)
		x->recordCurrent = 0;

	if (x->recordOn) {
		if (x->rec_ramping == 0)
			x->recordBuf[x->recordCurrent++] = sample;    // add sample
...
float getSamp(t_munger* x, double where) {
	...
	if (where < 0.)
		where = x->buflen + fmod(where, x->buflen);
	if (where >= x->buflen)
		where = fmod(where, x->buflen);
...
void ext_main(void* f) {
	t_class* c
		= class_new("munger~", (method)munger_new, (method)munger_free, (long)sizeof(t_munger), 0L, A_DEFFLOAT, A_DEFLONG, A_DEFLONG, 0);
...
void munger_setramp(t_munger* x, t_symbol* s, long argc, t_atom* argv) {
	...
	for (i = 0; i < argc; i++) {
		switch (argv[i].a_type) {
			case A_LONG:
				if (x->verbose > 0)
					object_post((t_object*)x, "setting ramp to: %ld ms", argv[i].a_w.w_long);
			...
			case A_FLOAT:
				if (x->verbose > 0)
					object_post((t_object*)x, "setting ramp to: %lf ms", argv[i].a_w.w_float);
...
void munger_scale(t_munger* x, t_symbol* s, long argc, t_atom* argv) {
	...
	if (x->verbose > 0)
		object_post((t_object*)x, "loading scale from input list");
	x->smoothPitch = 0;
...
void munger_spat(t_munger* x, t_symbol* s, long argc, t_atom* argv) {
	...
	for (i = j = 0; i < (argc - 1); i += 2) {
		...
		if (x->verbose > 0)
			object_post((t_object*)x, "channel gain %d = %f, spread = %f", j, x->channelGain[j], x->channelGainSpread[j]);
		j++;
	}
}
...
void munger_note(t_munger* x, t_symbol* s, long argc, t_atom* argv) {
	if (argc < 8) {
		object_error((t_object*)x,
			"munger: oneshot note needs 8 args -- transposition, gain, pan, attkT, decayT, susLevel, relT, direction [-1 or 0-1].");
		return;
	}
	...
	for (i = 0; i < argc; i++) {
		switch (argv[i].a_type) {
			case A_LONG:
				...
				x->oneshot = temp;
				if (x->verbose > 0)
					object_post((t_object*)x, "setting oneshot: %d", temp);
				break;
			case A_FLOAT:
				temp       = (int)argv[i].a_w.w_long;
				x->oneshot = temp;
	...
	for (i = 0; i < argc; i++) {
		switch (argv[i].a_type) {
			case A_LONG:
				...
				x->minsize = temp;
				if (x->verbose > 0)
					object_post((t_object*)x, "setting min grain size to: %f ms.", (x->minsize / x->srate_ms));
				break;
			case A_FLOAT:
				temp = x->srate_ms * argv[i].a_w.w_float;
				if (temp < (float)MINSIZE)
...
void munger_discretepan(t_munger* x, t_symbol* s, long argc, t_atom* argv) {
	...
	if (argc) {
		...
		if (temp < 0 || temp > 1) {
			...
			x->discretePan = temp;
			if (x->verbose > 0)
				object_post((t_object*)x, "setting discretepan to %d.", temp);
		}
	}
}
	...
	for (i = 0; i < argc; i++) {
		switch (argv[i].a_type) {
			case A_LONG:
				...
				x->voices = temp;
				if (x->verbose > 0)
					object_post((t_object*)x, "munger: setting current number of voices to: %d.", x->voices);
				break;
			case A_FLOAT:
	...
	for (i = 0; i < argc; i++) {
		switch (argv[i].a_type) {
			case A_LONG:
				...
				x->maxvoices = temp;
				if (x->verbose > 0)
					object_post((t_object*)x, "munger: setting max voices to: %d.", x->maxvoices);
	...
	for (i = 0; i < argc; i++) {
		switch (argv[i].a_type) {
			case A_LONG:
				...
				x->power = temp;
				if (x->verbose > 0)
					object_post((t_object*)x, "setting power: %d.", temp);
				break;
			case A_FLOAT:
				temp     = (int)argv[i].a_w.w_long;
				x->power = temp;
	...
	for (i = 0; i < argc; i++) {
		switch (argv[i].a_type) {
			case A_LONG:
				...
				if (x->verbose > 0)
					object_post((t_object*)x, "setting record: %d.", temp);
				break;
			case A_FLOAT:
				temp = (int)argv[i].a_w.w_float;
				// x->recordOn = temp;
	...
	for (i = 0; i < argc; i++) {
		switch (argv[i].a_type) {
			case A_LONG:
				...
				x->ambi = temp;
				if (x->verbose > 0)
					object_post((t_object*)x, "setting ambidirectional: %d.", temp);
				break;
			case A_FLOAT:
				temp    = (int)argv[i].a_w.w_float;
				x->ambi = temp;
	...
	for (i = 0; i < argc; i++) {
		switch (argv[i].a_type) {
			case A_LONG:
				...
				if (x->verbose > 0)
					object_post((t_object*)x, "setting gain to: %f.", temp);
				x->gain = temp;
				break;
			case A_FLOAT:
		...
		switch (argv[i].a_type) {
			case A_LONG:
				...
				if (temp < 0.)
					temp = -1.;
				if (x->verbose > 0)
					object_post((t_object*)x, "setting position to: %f.", temp);
				x->position = temp;
				break;
			case A_FLOAT:
				temp = argv[i].a_w.w_float;
	...
	for (i = 0; i < argc; i++) {
		switch (argv[i].a_type) {
			case A_LONG:
				temp = (float)argv[i].a_w.w_long;
				if (x->verbose > 0)
					object_post((t_object*)x, "setting rand_gain to: %f.", temp);
				x->randgain = temp;
				break;
			case A_FLOAT:
				temp = argv[i].a_w.w_float;
...
void munger_tempered(t_munger* x, t_symbol* s, long argc, t_atom* argv) {
	int i;
	if (x->verbose > 0)
		object_post((t_object*)x, "doing tempered scale.");
	x->smoothPitch = 0;
	x->scale_len   = 100;
...
void munger_smooth(t_munger* x, t_symbol* s, long argc, t_atom* argv) {
	if (x->verbose > 0)
		object_post((t_object*)x, "doing smooth scale.");
	x->smoothPitch = 1;
}
...
void* munger_new(double maxdelay, long channels, long numvoices) {
	...
	if (maxdelay < 100.)
		maxdelay = 3000.;    // set maxdelay to 3000ms by default
	object_post((t_object*)x, "maxdelay = %f milliseconds.", maxdelay);

	if (channels < 2)
		x->num_channels = 2;
	else
		x->num_channels = channels;
	if (x->num_channels > MAXCHANNELS)
		x->num_channels = MAXCHANNELS;
	object_post((t_object*)x, "number channels = %d.", x->num_channels);
	if (x->num_channels > 2)
		object_post((t_object*)x,
			"WARNING: more than 2 channels requested -- no sound will be outputted until you set the gains for each of the channels using "
			"the spatialize command.");
		...
		while (n--) {
			...
			for (i = 0; i < x->maxvoices; i++) {
				...
				if (x->gvoiceOn[i]) {
					...
					if (x->externalBuffer)
						samp = getExternalSamp(x, x->gvoiceCurrent[i], tab, frames, nc);
					else
						samp = getSamp(x, x->gvoiceCurrent[i]);
					if (!x->gvoiceADSRon[i])
						samp = envelope(x, i, samp) * x->gvoiceGain[i];
					else
						samp = samp * ADSR_ADRtick(&x->gvoiceADSR[i]) * x->gvoiceGain[i];
					...
					if (!x->gvoiceADSRon[i]) {
						if (++x->gvoiceDone[i] >= x->gvoiceSize[i])
							x->gvoiceOn[i] = 0;
					}
					else {
						if (ADSR_getState(&x->gvoiceADSR[i]) == DONE)
							x->gvoiceOn[i] = 0;
					}
				}
			}
