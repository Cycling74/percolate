...
void bowed_perform64(
	t_bowed* x, t_object* dsp64, double** ins, long numins, double** outs, long numouts, long sampleframes, long flags, void* userparam);
...
void bowed_perform64(
	...
	while (n--) {
		...
		bridgeRefl = -OnePole_tick(&x->reflFilt, x->bridgeDelay.lastOutput); /* Bridge Reflection      */
		...
		if (va > 0.) {
			DLineL_setDelay(&x->neckDelay, (x->baseDelay * (1.0 - x->betaRatio)) + (x->baseDelay * va * vib_tick(x)));
		}
