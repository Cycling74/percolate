...
void flapper_perform64(
	t_flapper* x, t_object* dsp64, double** ins, long numins, double** outs, long numouts, long sampleframes, long flags, void* userparam);
...
void flapper_assist(t_flapper* x, void* b, long m, long a, char* s) {
	...
	if (m == 1) {
		if (a == 0)
			sprintf(s, "trigger (signal/float, 0/1)");
		if (a == 1)
			sprintf(s, "input (signal)");
		if (a == 2)
			sprintf(s, "playback speed (signal/float)");
	}
	if (m == 2) {
		if (a == 0)
			sprintf(s, "out left (signal)");
		if (a == 1)
			sprintf(s, "out right (signal)");
	}
}
...
void queue_put(t_flapper* x, int v) {
	...
	if (x->fq_tail > MAX_FLAPS)
		x->fq_tail = 0;
}

int queue_get(t_flapper* x) {
	int t = x->flap_queue[x->fq_head++];
	if (x->fq_head > MAX_FLAPS)
		x->fq_head = 0;
	return t;
}
	...
	for (i = 0; i < argc; i++) {
		switch (argv[i].a_type) {
			case A_LONG:
				...
				if (x->overlap_len_default <= 1.)
					x->overlap_len_default = 1.;
				break;
			case A_FLOAT:
				temp                   = argv[i].a_w.w_float;
