...
void weave_perform64(
	t_weave* x, t_object* dsp64, double** ins, long numins, double** outs, long numouts, long sampleframes, long flags, void* userparam);
...
void weave_perform64(
	...
	while (--n) {    // loop executes a single vector
		if (*++in > 0.0) {
			...
			cval_new = 0;            // check current sample and assign its polarity to cval_new
