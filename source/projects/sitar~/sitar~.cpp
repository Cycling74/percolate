...
void sitar_dsp(t_sitar *x, t_signal **sp, short *count); 
t_int *sitar_perform(t_int *w);
...
//for custom messages
void sitar_setpower(t_sitar *x, Symbol *s, short argc, Atom *argv);
void sitar_controlchange(t_sitar *x, Symbol *s, short argc, Atom *argv);
void sitar_noteon(t_sitar *x, Symbol *s, short argc, Atom *argv);
void sitar_noteoff(t_sitar *x, Symbol *s, short argc, Atom *argv);
...
extern "C" int main(void)
{
    ...
    setup((struct messlist **)&sitar_class, (method)sitar_new, (method)sitar_free, (short)sizeof(t_sitar), 0L, A_DEFLONG, A_DEFLONG, 0);
   
	//standard messages; don't change these  
    addmess((method)sitar_dsp, "dsp", A_CANT, 0);
    addmess((method)sitar_assist,"assist", A_CANT,0);
    
    //our own messages
    addmess((method)sitar_setpower, "power", A_GIMME, 0);
    addmess((method)sitar_controlchange, "control", A_GIMME, 0);
    addmess((method)sitar_noteon, "noteon", A_GIMME, 0);
    addmess((method)sitar_noteoff, "noteoff", A_GIMME, 0);

    //so we know what to do with floats that we receive at the inputs
    addfloat((method)sitar_float);
...
void *sitar_new(long xD, long yD)
{
    ...
    t_sitar *x = (t_sitar *)newobject(sitar_class);
...
void sitar_dsp(t_sitar *x, t_signal **sp, short *count)
{
	...
	dsp_addv(sitar_perform, (x->num_inputs + x->num_outputs + 2), dsp_add_args); //add them to the signal chain
