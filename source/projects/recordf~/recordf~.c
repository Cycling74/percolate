...
void recordf_perform64(
	t_recordf* x, t_object* dsp64, double** ins, long numins, double** outs, long numouts, long sampleframes, long flags, void* userparam);
...
void recordf_in0(t_recordf* x, long n) {
	x->record = n;
	if (!x->append)
		x->index = 0;    // reset recording point to beginning of loop
}
...
void recordf_perform64(
	...
	if (x->length != 0)
		one_over_length = 1. / (t_double)x->length;
	else
		one_over_length = 1. / frames;
