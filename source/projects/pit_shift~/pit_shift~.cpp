...
void pit_shift_perform64(t_pit_shift* x, t_object* dsp64, double** ins, long numins, double** outs, long numouts, long sampleframes,
...
void ext_main(void* p) {
	t_class* c
		= class_new("pit_shift~", (method)pit_shift_new, (method)pit_shift_free, (long)sizeof(t_pit_shift), 0L, A_DEFLONG, A_DEFLONG, 0);
	...
	class_addmethod(c, (method)pit_shift_clear, "clear", A_GIMME, 0);
	class_dspinit(c);
...
void pit_shift_perform64(t_pit_shift* x, t_object* dsp64, double** ins, long numins, double** outs, long numouts, long sampleframes,
	...
	while (n--) {    // this is where the action happens..... let's make something up
		...
		if (x->in_connected[0])
			temp = *in[0]++;    // use the signal vector if there is one
		else
			temp = *in[0];    // otherwise use the global variable
