...
void  scrub_assist(t_scrub* x, void* b, long io, long index, char* s);
...
void scrub_perform64(
	t_scrub* x, t_object* dsp64, double** ins, long numins, double** outs, long numouts, long sampleframes, long flags, void* userparam);
	...
	for (i = 0; i < argc; i++) {
		switch (argv[i].a_type) {
			case A_LONG:
				...
				if (x->recordenv > 0.5 * x->delaylen)
					x->recordenv = 0.5 * x->delaylen;
	...
	for (i = 0; i < argc; i++) {
		switch (argv[i].a_type) {
			case A_LONG:
				...
				if (x->delaylen > x->buflen)
					x->delaylen = x->buflen;
				if (x->delaylenUpdate > x->buflen)
...
float getSamp0(t_scrub* x) {
	...
	while (x->where0 < 0.)
		x->where0 += x->delaylen0;
	while (x->where0 >= x->delaylen0)
		x->where0 -= x->delaylen0;
...
float getSamp2(t_scrub* x) {
	...
	while (x->where2 < 0.)
		x->where2 += x->delaylen2;
	while (x->where2 >= x->delaylen2)
		x->where2 -= x->delaylen2;
...
float getSamp1(t_scrub* x) {
	...
	while (x->where1 < 0.)
		x->where1 += x->delaylen1;
	while (x->where1 >= x->delaylen1)
		x->where1 -= x->delaylen1;
...
float recordEnvelope(t_scrub* x, float sample) {
	...
	if (done < x->recordenv)
		sample *= done * x->recordenvInv;
	else if (tail < x->recordenv)
		sample *= tail * x->recordenvInv;

	return sample;
}

float attackEnvelope(t_scrub* x, float sample) {
	if (x->recordPoint < x->overlap)
		sample *= x->recordPoint * x->overlapInv;
	else
		sample = 1.;
...
float decayEnvelope(t_scrub* x, float sample) {
	...
	if (tail < x->overlap)
		sample *= (float)tail * x->overlapInv;
	else
		sample = 1.;
	...
	if (x->power) {
		while (n--) {
			...
			if (x->recordPoint == 0 && x->record) {
				...
				if (x->recordenv > 0.5 * x->recordlen)
					x->recordenv = 0.5 * x->recordlen;
				x->recordenvInv = 1. / x->recordenv;

				x->overlap = x->overlapUpdate;
				if (x->overlap > 0.5 * x->recordlen)
					x->overlap = 0.5 * x->recordlen;
				x->overlapInv = 1. / x->overlap;
